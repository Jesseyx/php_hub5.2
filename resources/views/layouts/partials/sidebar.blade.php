    ...
    @if (Route::currentRouteName() == 'topics.index')
        ...
        <div class="panel panel-default corner-radius">
            <div class="panel-body text-center">
                <div class="btn-group">
                    <a class="btn btn-primary btn-lg btn-inverted" href="{{ route('topics.create') }}">
            ...
            @if(isset($banners['sidebar-sponsor']))
                @foreach($banners['sidebar-sponsor'] as $banner)
                    <a class="sidebar-sponsor-link" href="{{ $banner->link }}" target="_blank">
                        <img class="popover-with-html" src="{{ $banner->image_url }}" data-content="{{ $banner->title }}" width="100%">
                    </a>
                @endforeach
            @endif
        </div>
